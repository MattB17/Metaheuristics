...
import pytest
import pandas as pd
from metaheuristics.optimization_functions import quadratic_assignment_problem_objective
...
def test_with_empty_dataframes():
    ...
    result = quadratic_assignment_problem_objective(dist_matrix, flow_matrix, [])
    assert result == 0
...
def test_with_dataframe_having_one_element():
    ...
    result = quadratic_assignment_problem_objective(dist_matrix, flow_matrix, ["A"])
    assert result == 6
    
    
def test_with_non_trivial_dataframes_no_reindexing(mock_flow_matrix, mock_dist_matrix):
    facility_order = ["X", "Y", "Z"]
    result = quadratic_assignment_problem_objective(
